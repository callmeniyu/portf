...
import type { Highlight, HighlightPage, ExperiencePage, SkillPage, ServicePage, EducationPage, ContactPage, AboutPage } from '@/lib/data'
...
export default function StoryViewer({ highlights, initialHighlightId, onClose }: StoryViewerProps) {
  ...
  const goNext = useCallback(() => {
    if (currentPage < pages.length - 1) { setCurrentPage(p => p + 1); setProgress(0) }
    else if (currentHighlight < highlights.length - 1) { setCurrentHighlight(h => h + 1); setCurrentPage(0); setProgress(0) }
    else { onClose() }
  }, [currentPage, pages.length, currentHighlight, highlights.length, onClose])

  const goPrev = useCallback(() => {
    if (currentPage > 0) { setCurrentPage(p => p - 1); setProgress(0) }
    else if (currentHighlight > 0) { setCurrentHighlight(h => h - 1); setCurrentPage(0); setProgress(0) }
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      const p = Math.min(((now - startTimeRef.current) / STORY_DURATION) * 100, 100)
      setProgress(p)
        ...
        <div className={styles.header}>
          <div className={styles.highlightInfo}>
            ...
            <div>
              ...
              <div className={styles.hlTime}>{currentPage + 1}/{pages.length}</div>
            </div>
          </div>
          <div className={styles.headerActions}>
            <button className={styles.headerBtn} onClick={() => { pausedProgressRef.current = progress; setPaused(p => !p) }} aria-label="Pause/resume">
              {paused
...
function StoryPageContent({ page, gradient }: { page: HighlightPage; gradient: string }) {
  switch (page.type) {
    case 'about':      return <AboutStory page={page as AboutPage} />
...
function AboutStory({ page }: { page: AboutPage }) {
  return (
    <div className={styles.storyCard} style={{ background: page.accent }}>
      <div className={styles.aboutContent}>
        <div className={styles.aboutEmoji}>✨</div>
        <div className={styles.storyBadge}>Portfolio</div>
        <h2 className={styles.storyTitle}>{page.title}</h2>
        <p className={styles.aboutSubtitle}>{page.subtitle}</p>
        <p className={styles.aboutBody}>{page.body}</p>
        <div className={styles.aboutHighlight}>
          <span className={styles.highlightPill}>{page.highlight}</span>
        </div>
      </div>
    </div>
  )
    ...
    <div className={styles.storyCard}>
      ...
      {page.image && (
        <div className={styles.expBgWrap}>
          <Image src={page.image} alt={page.company} fill className={styles.expBg} sizes="470px" />
          <div className={styles.expBgOverlay} style={{ background: `linear-gradient(to top, rgba(0,0,0,0.97) 0%, rgba(0,0,0,0.6) 60%, rgba(0,0,0,0.2) 100%)` }} />
        </div>
      )}
      {!page.image && <div className={styles.expSolidBg} style={{ background: gradient }} />}
      ...
      <div className={styles.expContent}>
        ...
        <p className={styles.expMeta}>{page.period} · {page.location}</p>
        <p className={styles.expDesc}>{page.description}</p>
        <div className={styles.techRow}>
          {page.tech.map(t => <span key={t} className={styles.techChip}>{t}</span>)}
        </div>
      </div>
    </div>
  )
...
function SkillStory({ page, gradient }: { page: SkillPage; gradient: string }) {
  return (
    <div className={styles.storyCard} style={{ background: 'var(--ig-bg)' }}>
      <div className={styles.skillGradTop} style={{ background: gradient }} />
      <div className={styles.skillContent}>
        <div className={styles.storyBadgeColored} style={{ background: gradient }}>
          {page.category}
        </div>
        <h2 className={styles.skillTitle}>{page.title}</h2>
        <div className={styles.skillList}>
          {page.skills.map((skill, i) => (
            <div key={skill.name} className={styles.skillItem} style={{ animationDelay: `${i * 0.08}s` }}>
              <div className={styles.skillHeader}>
                <span className={styles.skillName}>{skill.icon} {skill.name}</span>
...
function ServiceStory({ page, gradient }: { page: ServicePage; gradient: string }) {
  return (
    <div className={styles.storyCard}>
      <div className={styles.srvBackground} style={{ background: gradient }} />
      <div className={styles.srvContent}>
        <div className={styles.srvIconCircle}>{page.icon}</div>
...
function EducationStory({ page, gradient }: { page: EducationPage; gradient: string }) {
  return (
    <div className={styles.storyCard}>
      <div className={styles.eduBackground} style={{ background: gradient }} />
      <div className={styles.eduContent}>
        <div className={styles.eduIcon}>{page.icon}</div>
...
function ContactStory({ page, gradient }: { page: ContactPage; gradient: string }) {
  return (
    <div className={styles.storyCard}>
      <div className={styles.cntBackground} style={{ background: gradient }} />
      <div className={styles.cntContent}>
        <div className={styles.cntEmoji}>📬</div>
        <div className={styles.storyBadge}>Get In Touch</div>
        <h2 className={styles.storyTitle}>{page.title}</h2>
        <div className={styles.contactGrid}>
          {[
            { icon: '📧', label: 'Email', value: page.email },
            { icon: '📱', label: 'Phone', value: page.phone },
