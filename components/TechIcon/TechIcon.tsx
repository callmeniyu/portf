...
const ICON_MAP: Record<string, IconDef> = {
  ...
  'Zustand':            { Icon: FiBox,          color: '#ff9f43' },
  'React Query':        { Icon: FiRefreshCw,    color: '#FF4154' },
  'GraphQL':            { Icon: SiGraphql,      color: '#E10098' },
  'Node.js':            { Icon: SiNodedotjs,   color: '#339933' },
  'Express.js':         { Icon: SiExpress,      color: '#aaaaaa' },
  'Python':             { Icon: SiPython,       color: '#3776AB' },
  'Helmet':             { Icon: FaShieldAlt,    color: '#55acee' },
  'REST APIs':          { Icon: FiLink,         color: '#0095f6' },
  'PostgreSQL':         { Icon: SiPostgresql,  color: '#4169E1' },
  'MongoDB':            { Icon: SiMongodb,      color: '#47A248' },
  'Redis':              { Icon: SiRedis,        color: '#DC382D' },
  'Prisma ORM':         { Icon: SiPrisma,       color: '#888888' },
  'Firebase':           { Icon: SiFirebase,     color: '#FFCA28' },
  'Appwrite':           { Icon: SiAppwrite,     color: '#FD366E' },
  'Supabase':           { Icon: SiSupabase,     color: '#3ECF8E' },
  'shadcn':             { Icon: SiShadcnui,     color: '#ffffff' },
  'Docker':             { Icon: SiDocker,       color: '#2496ED' },
  'CI/CD':              { Icon: FiRefreshCw,    color: '#00b894' },
  'Vercel / Netlify':   { Icon: SiVercel,       color: '#ffffff' },
  'GitHub':             { Icon: SiGithub,       color: '#ffffff' },
  'Expo':               { Icon: SiExpo,         color: '#ffffff' },
  'Stripe':             { Icon: SiStripe,       color: '#635BFF' },
  'Product Design':     { Icon: SiFigma,        color: '#F24E1E' },
  'System Architecture':{ Icon: FiBox,          color: '#a29bfe' },
  'Product Architecture': { Icon: FiBox,        color: '#a29bfe' },
  'Team Leadership':    { Icon: FiSmartphone,   color: '#fd79a8' },
  'Frontend Development': { Icon: SiReact,      color: '#61DAFB' },
...
export default function TechIcon({ name, size = 18 }: TechIconProps) {
  // Special cases for icons not available in Si v5
  if (name === 'LinkedIn') return <LinkedInSVG size={size} />
  if (name === 'AWS') return <AWSIcon size={size} />

  const def = ICON_MAP[name]
  if (!def) return <FiCode size={size} color="#888" />
